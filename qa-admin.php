...
if ($hasUsersTable && $hasRolesTable) {
    ...
    if ($res) {
        ...
        $totalQaAdmins = (int)($row['total_count'] ?? 0);
    }
    ...
    $sql = "
        ...
        FROM users u
        INNER JOIN roles r ON r.id = u.current_role_id
        WHERE r.role_code = 'qa_admin' AND u.status = 'active'
    ";
    $res = mysqli_query($conn, $sql);
    if ($res) {
        $row = mysqli_fetch_assoc($res);
        $activeQaAdmins = (int)($row['total_count'] ?? 0);
    }
    ...
    if ($res) {
        $row = mysqli_fetch_assoc($res);
        $inactiveQaAdmins = (int)($row['total_count'] ?? 0);
    }
...
/*
|--------------------------------------------------------------------------
| RECENT ROLE CHANGES TO QA ADMIN
|--------------------------------------------------------------------------
*/
...
<head>
  ...
  <title>CorePlx Quality DMS - QA Admin</title>
...
<main class="app-shell">
  <div class="content-wrap px-4 py-4 mx-auto">
    <div class="mb-4">
      <h1 class="page-title mb-2">QA Admin Oversight</h1>
      <p class="page-subtitle mb-0">Administer controlled documents, permissions, and quality oversight activities.</p>
    </div>
    ...
    <div class="row g-3 mb-3">
      <div class="col-sm-6 col-xl-4">
        <div class="card cp-card h-100">
          <div class="card-body">
            <div class="stat-number"><?php echo (int)$totalQaAdmins; ?></div>
            <div class="stat-label">Total QA Admins</div>
          </div>
        </div>
      </div>
      ...
      <div class="col-sm-6 col-xl-4">
        <div class="card cp-card h-100">
          <div class="card-body">
            <div class="stat-number"><?php echo (int)$activeQaAdmins; ?></div>
            <div class="stat-label">Active QA Admins</div>
          </div>
        </div>
      </div>
      ...
      <div class="col-sm-6 col-xl-4">
        <div class="card cp-card h-100">
          <div class="card-body">
            <div class="stat-number"><?php echo (int)$inactiveQaAdmins; ?></div>
        ...
        <div class="card cp-card h-100">
          <div class="card-body">
            ...
            <ul class="small text-secondary mb-0">
              <li>Create, update, and retire documents.</li>
              <li>Assign read-only access to repository.</li>
              <li>View quality workflows and audit trails.</li>
              <li>All role assignments must be logged.</li>
            </ul>
          </div>
        </div>
      </div>
        ...
        <div class="card cp-card h-100">
          <div class="card-body">
            ...
            <div class="mt-3 d-flex flex-wrap gap-2">
              <span class="soft-badge">QA Admin Users: <?php echo (int)$totalQaAdmins; ?></span>
              <span class="soft-badge success">Active: <?php echo (int)$activeQaAdmins; ?></span>
              <span class="soft-badge secondary">Inactive: <?php echo (int)$inactiveQaAdmins; ?></span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="row g-3 mt-1">
      <div class="col-xl-7">
        <div class="card cp-card h-100">
          <div class="card-body">
            <h2 class="card-title mb-1">QA Admin Users</h2>
            <p class="card-subtitle mb-3">Recent users currently assigned to QA Admin role.</p>
            ...
            <div class="table-responsive">
              <table class="table align-middle mb-0">
                ...
                <tbody>
                  <?php if (!empty($recentQaUsers)): ?>
                    <?php foreach ($recentQaUsers as $user): ?>
                      <?php
                  ...
                  <?php else: ?>
                    <tr>
                      <td colspan="5" class="text-center text-muted py-4">No QA Admin users found.</td>
                    </tr>
                  <?php endif; ?>
                </tbody>
              </table>
      ...
      <div class="col-xl-5">
        <div class="card cp-card h-100">
          <div class="card-body">
            <h2 class="card-title mb-1">Recent QA Admin Role Changes</h2>
            <p class="card-subtitle mb-3">Latest role updates where QA Admin access was assigned.</p>

            <?php if (!empty($recentQaRoleChanges)): ?>
              <?php foreach ($recentQaRoleChanges as $item): ?>
                <?php
              ...
              <?php endforeach; ?>
            <?php else: ?>
              <div class="small text-secondary">No recent QA Admin role changes found.</div>
            <?php endif; ?>
          </div>
        </div>
      </div>
